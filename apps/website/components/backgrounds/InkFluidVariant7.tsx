...
float fluidWave(vec2 uv, float time) {
    vec2 flow1 = vec2(
        fbm(uv * 0.4 + vec2(time * 0.25, time * 0.3)),
        fbm(uv * 0.4 + vec2(time * 0.28, time * 0.33))
    );
    
    float wave1 = fbm(uv * 1.0 + flow1 * 2.5 + vec2(time * 0.35, time * 0.4));
    float wave2 = fbm(uv * 1.5 - flow1 * 2.0 + vec2(time * 0.4, time * 0.45));
    float wave3 = fbm(uv * 2.0 + flow1 * 3.0 + vec2(time * 0.45, time * 0.5));
...
void main() {
    ...
    // Сильные блики на поверхности
    float reflection = pow(max(0.0, fluid - 0.8), 1.2) * 3.5;
    color = mix(color, white, reflection * 0.4);
...
export default function InkFluidVariant7() {
