...
float fluidWave(vec2 uv, float time) {
    vec2 flow1 = vec2(
        fbm(uv * 0.25 + vec2(time * 0.12, time * 0.15)),
        fbm(uv * 0.25 + vec2(time * 0.14, time * 0.17))
    );
    
    float wave1 = fbm(uv * 0.6 + flow1 * 1.5 + vec2(time * 0.15, time * 0.18));
    float wave2 = fbm(uv * 0.8 - flow1 * 1.2 + vec2(time * 0.18, time * 0.22));
    
    float combined = (wave1 * 0.6 + wave2 * 0.4);
    combined = pow(combined * 0.5 + 0.5, 1.1);
    return combined;
}
...
export default function InkFluidVariant5() {
