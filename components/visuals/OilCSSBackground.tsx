...
export default function OilCSSBackground() {
  ...
  useEffect(() => {
    const checkPerformance = () => {
      ...
      // Проверяем количество ядер (если <4 - слабое устройство)
  ...
  useEffect(() => {
    if (isLowPerf) return; // Отключаем отслеживание мыши на слабых устройствах
    ...
    // Плавная интерполяция позиции (60fps -> меньше ререндеров)
    const animate = () => {
      const lerp = 0.08; // Плавность
  ...
  // Мемоизированный статичный градиент для слабых устройств
  ...
  return (
    <div className="absolute inset-0 overflow-hidden will-change-transform">
      ...
      {/* Анимированные волны - зацикленные CSS анимации (GPU) */}
      ...
      {/* Металлический блик - только для мощных устройств */}
      {!isLowPerf && (
        <div 
      ...
      {/* Зацикленные CSS анимации на GPU */}
      <style jsx>{`
        .oil-wave {
          position: absolute;
          inset: -20%;
