...
float fluidWave(vec2 uv, float time) {
    vec2 flow1 = vec2(
        fbm(uv * 0.6 + vec2(time * 0.3, time * 0.35)),
        fbm(uv * 0.6 + vec2(time * 0.32, time * 0.38))
    );
    
    float wave1 = fbm(uv * 1.5 + flow1 * 2.0 + vec2(time * 0.4, time * 0.45));
    float wave2 = fbm(uv * 2.0 - flow1 * 1.8 + vec2(time * 0.45, time * 0.5));
    float wave3 = fbm(uv * 2.5 + flow1 * 2.5 + vec2(time * 0.5, time * 0.55));
...
export default function InkFluidVariant4() {
