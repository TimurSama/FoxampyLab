...
float fbm(vec2 p) {
    ...
    float amplitude = 0.5;
    float frequency = 1.0;
    // Less octaves for smoother, more "liquid" look, more for detail
    for (int i = 0; i < 4; i++) {
...
// Pseudo-environment map for metallic look
...
void main() {
    ...
    // Mouse interaction (push/ripple)
    float mouseDist = length(uv - mouse);
    // Smooth, wide soft interaction field
    float mouseForce = exp(-mouseDist * 3.0) * uInteractionStrength;
    
    // Domain warping adapted for flow
    vec2 q = vec2(0.);
    q.x = fbm(uv + 0.00 * t);
    q.y = fbm(uv + vec2(1.0));
    ...
    // Calculate normal from height map (f) for lighting
    vec2 e = vec2(0.01, 0.0);
    float h = f;
    float h_r = fbm(uv + e.xy + r);
    float h_u = fbm(uv + e.yx + r);
    vec3 normal = normalize(vec3(h - h_r, h - h_u, 0.1 * uViscosity)); // Adjust Z for "depth"

    // Lighting (pseudo-specular/metallic)
    vec3 viewDir = vec3(0.0, 0.0, 1.0); // Orthographic view approx
    vec3 lightDir = normalize(vec3(0.5, 0.5, 1.0));
    
    // Specular highlight (Blinn-Phong) - Sharper for "veins"
    vec3 halfDir = normalize(lightDir + viewDir);
    float NdotH = max(0.0, dot(normal, halfDir));
    float specular = pow(NdotH, 120.0 * uMetallic); // Higher power = thinner lines
    
    // Color mixing based on flow density (f) and height
    vec3 color = mix(uColor1, uColor2, smoothstep(0.2, 0.8, f));
    // Sharp transition for veins
    color = mix(color, uColor3, smoothstep(0.45, 0.5, length(q)));

    // Add metallic sheen - subtle but visible veins
    color += vec3(specular) * 0.4;
    
    // "Rim" light effect based on normal variation
...
export default function LiquidMercury({ config }: LiquidMercuryProps) {
    ...
    const defaultConfig = {
        color1: '#000000',
        color2: '#0a0a0a',
        color3: '#505050', // Visible dark silver
        speed: 0.05,
        viscosity: 1.0,
        metallic: 0.6,
    ...
    useFrame((state) => {
        ...
        // Mouse smoothing could be added here, but direct mapping feels punchier for "liquid"
        if (state.pointer) {
            // Convert normalized pointer (-1 to +1) to pixel coords for shader
            // (Noting shader expects pixels, although normalized UVs are often easier, 
            // keep enabling "pixel perfect" mouse tracking relative to resolution)
            const x = (state.pointer.x + 1) * 0.5 * size.width;
            const y = (state.pointer.y + 1) * 0.5 * size.height;

            // Simple lerp for smoother interaction?
